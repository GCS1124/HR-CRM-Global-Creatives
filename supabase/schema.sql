...
begin
  ...
  values (
    ...
    v_full_name,
    case
      when lower(v_email) = 'test@crm.co.in' then 'admin'
      else 'employee'
    end
  )
...
begin
  ...
  values (
    ...
    case
      when lower(new.email) = 'test@crm.co.in' then 'admin'
      else 'employee'
    end
  )
  on conflict (id) do update
...
select
  ...
  coalesce(u.raw_user_meta_data ->> 'full_name', split_part(u.email, '@', 1)),
  case
    when lower(u.email) = 'test@crm.co.in' then 'admin'
    else 'employee'
  end
from auth.users u
on conflict (id) do update
set
  ...
  full_name = coalesce(excluded.full_name, public.profiles.full_name),
  role = case
    when lower(excluded.email) = 'test@crm.co.in' then 'admin'
    else 'employee'
  end;

update public.profiles
set role = case
  when lower(email) = 'test@crm.co.in' then 'admin'
  else 'employee'
end;
...
begin
  if not exists (
    ...
    alter table public.profiles
      ...
      check (role <> 'admin' or lower(email) = 'test@crm.co.in');
  end if;
end $$;

create unique index if not exists profiles_single_admin_role on public.profiles((role)) where role = 'admin';
